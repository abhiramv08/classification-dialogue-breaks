...
def augment_text_by_mlm(text, mask_rate=0.3, max_predictions_per_seq=20):
    ...
    candidate_indices = [i for i, token in enumerate(tokens) if token not in ['<s>', '</s>']]  # Roberta uses <s> and </s>
    ...
    augmented_text = tokenizer.convert_tokens_to_string(tokens).replace('<s>', '').replace('</s>', '')
...
df_augmented.to_csv('train_augmented_mlm.csv', index=False)
